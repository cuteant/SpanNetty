  ...
  public sealed class TlsHandler : ByteToMessageDecoder
  {
        ...
        case TaskStatus.Faulted:
          {
            ...
            TlsHandlerState oldState = self._state;
            Contract.Assert(!oldState.HasAny(TlsHandlerState.AuthenticationCompleted));
            self._state = (oldState | TlsHandlerState.FailedAuthentication) & ~TlsHandlerState.Authenticating;
    ...
    private sealed class MediationStream : Stream
    {
      ...
      public override Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
          => _owner._capturedContext.WriteAndFlushAsync(Unpooled.WrappedBuffer(buffer, offset, count));
      ...
      {
        Task task = _owner._capturedContext.WriteAndFlushAsync(Unpooled.WrappedBuffer(buffer, offset, count));
        switch (task.Status)
        {
          case TaskStatus.RanToCompletion:
