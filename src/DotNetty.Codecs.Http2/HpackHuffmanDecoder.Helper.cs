...
{
    partial class HpackHuffmanDecoder
    {
        /* Scroll to the bottom! */
        private static readonly byte HUFFMAN_COMPLETE = 1;
        private static readonly byte HUFFMAN_EMIT_SYMBOL = 1 << 1;
        private static readonly byte HUFFMAN_FAIL = 1 << 2;

        private static readonly int HUFFMAN_COMPLETE_SHIFT = HUFFMAN_COMPLETE << 8;
        private static readonly int HUFFMAN_EMIT_SYMBOL_SHIFT = HUFFMAN_EMIT_SYMBOL << 8;
        private static readonly int HUFFMAN_FAIL_SHIFT = HUFFMAN_FAIL << 8;

        // A table of byte tuples (state, flags, output).   They are packed together as:
        // state<<16 + flags<<8 + output
        private static readonly int[] HUFFS = new int[] {
