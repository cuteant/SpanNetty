...
namespace DotNetty.Transport.Libuv.Requests
{
    ...
    internal sealed class WriteRequest : ScheduleRequest
    {
            ...
            : base(requestType)
        {
            ...
            var addr = _requestContext.Handle;
        ...
        internal void Prepare(IByteBuffer buf, Action<WriteRequest, Exception> callback)
        {
            ...
            {
                var arraySegment = buf.GetIoBuffer();
            ...
            var bufsPin = GCHandle.Alloc(_bufsArray, GCHandleType.Pinned);
        ...
        {
            var baseOffset = _bufs + BufferSize * _count;
            ++_count;
