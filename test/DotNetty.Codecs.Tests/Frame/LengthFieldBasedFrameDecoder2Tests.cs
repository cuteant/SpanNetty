...
namespace DotNetty.Codecs.Tests.Frame
{
    ...
    public class LengthFieldBasedFrameDecoder2Tests
    {
        [Fact]
        public void FailSlowTooLongFrameRecovery()
        {
            var ch = new EmbeddedChannel(new LengthFieldBasedFrameDecoder2(5, 0, 4, 0, 4, false));
            for (int i = 0; i < 2; i++)
            {
        ...
        {
            var ch = new EmbeddedChannel(new LengthFieldBasedFrameDecoder2(5, 0, 4, 0, 4));
