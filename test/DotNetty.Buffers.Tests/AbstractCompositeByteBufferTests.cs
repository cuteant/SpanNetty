...
namespace DotNetty.Buffers.Tests
{
    ...
    public abstract class AbstractCompositeByteBufferTests : AbstractByteBufferTests
    {
        ...
        public void RangedConsolidation()
        {
            ...
            Assert.Equal(3, buf.NumComponents);
#if TEST40
            Assert.True(Unpooled.WrappedBuffer(new byte[] { 1 }).Equals(buf[0]));
            Assert.True(Unpooled.WrappedBuffer(new byte[] { 2, 3, 4, 5, 6 }).Equals(buf[1]));
            Assert.True(Unpooled.WrappedBuffer(new byte[] { 7, 8, 9, 10 }).Equals(buf[2]));
#else
        ...
        [Fact]
        public void AllocatorIsSameWhenCopy() => this.AllocatorIsSameWhenCopyInternal(false);

        [Fact]
        public void AllocatorIsSameWhenCopyUsingIndexAndLength() => this.AllocatorIsSameWhenCopyInternal(true);

        void AllocatorIsSameWhenCopyInternal(bool withIndexAndLength)
        {
            IByteBuffer buffer = this.NewBuffer(8);
            buffer.WriteZero(4);
            IByteBuffer copy = withIndexAndLength ? buffer.Copy(0, 4) : buffer.Copy();
#if TEST40
            Assert.True(buffer.Equals(copy));
#else
